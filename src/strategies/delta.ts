...
export class DeltaWeave implements IWeaveStrategy {
     ...
     */
    public decompress(compressed: Uint8Array, originalLength: number): Uint8Array {
        ...
        if (mode === 0x00) {
            if (payload.length !== originalLength) throw new Error('DeltaWeave: STORE length mismatch');
