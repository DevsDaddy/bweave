...
export class BWeave {
  ...
  public compress(data: Uint8Array): Uint8Array {
    if (data.length === 0)
      return BWeaveUtils.writeHeader(
        BWeaveMode.STORE,
        0,
        new Uint8Array(0),
        this.options.checksum
      );
    const mode = this.options.autoMode
      ? BWeaveUtils.detectBestMode(data, this.strategies)
      : this.options.forcedMode;
